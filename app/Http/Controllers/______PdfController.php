...
class PdfController extends Fpdf
{
  ...
  public function boletaTitulo($object = array())
  {
    ...
    $this->Cell(0, $this->height + 2, utf8_decode($object['seccion']['anio']), 'B', 1, 'C');
  }
  ...
  public function tableNotesBoleta($object = array())
    ...
    $this->Cell(70);    
    $this->Cell(14.67, $this->height, "A1", 1, 0, 'C', 1);
    $this->Cell(14.67, $this->height, "A2", 1, 0, 'C', 1);
    $this->Cell(14.67, $this->height, "A3", 1, 0, 'C', 1);
    $this->Cell(15.99, $this->height, "PR.", 1, 0, 'C', 1);
  ...
  public function avg($array = array())
  {
    ...
    $this->SetFont('Arial','B', 10);
    $this->Cell(15.99, $this->height + 1, number_format($promedio, 2) , 1, 0, 'C');
  ...
  public function cuadroFinal(array $args)
  {
    ...
    $this->getHeaderCuadroFinal();
    $this->getCuadroHeaderFinal();

    $this->CuadroDeDatos($args['items'], array( "init" => 1, "max" => 23 ));
    $this->CuadroEstadistica();

    // segunda pagina
    $this->AddPage();
    $this->getCuadroHeaderFinal(array(), false, 1);
    if(count($args['items']) > 22)
		  $this->CuadroDeDatos($args['items'], array("init" => 24, "max" => 50));
	  else
		  $this->CuadroDeDatos(array(), array("init" => 24, "max" => 50));
    
    $this->promedioGlobalModificado($args['promedios'][0]);
    $this->getDatosSchoolModificado();
  }
  ...
  public function getDatosSchoolModificado($array = array(), $institucion = array())
  {
    ...
    $this->Cell(7, 0.45, utf8_decode(@$institucion['lugar_fisrt']), 'B', 0, 'L');
    $this->SetXY(26.0, 1.6);
    $this->Cell(8.2, 0.45, utf8_decode(@$institucion['lugar_second']), 'B');

    $this->SetXY(26.0, 3.2);
    $this->Cell(1.2, 0.45, 'Fecha: ', 0, 0, 'L');
    $this->Cell(7, 0.45, utf8_decode(@$institucion['fecha_cuadro']), 'B', 0, 'L');
    $this->SetXY(26.0, 3.8);
    $this->Cell(1.9, 0.45, '', 'B', 0, 'L');
    $this->Cell(6.3, 0.45, utf8_decode(@$institucion['fecha_cuadro_second']), 'B');
    ...
    $this->Cell(4, 0.45, utf8_decode(@$array['nombre_docente']), 'B');

    $this->SetXY(26.0, 6.15);
    $this->Cell(2.6, 0.45, '', 'B', 0, 'L');
    $this->Cell(5.6, 0.45, utf8_decode(@$array['apellido_docente']), 'B', 0, 'L');
    ...
    $this->Cell(4.4, 0.45,utf8_decode(@$institucion['nombre_director']), 'B');

    $this->SetXY(26.0, 9);
    $this->Cell(3.8, 0.45, '', 'B', 0, 'L');
    $this->Cell(4.4, 0.45, utf8_decode(@$institucion['apellidos_director']), 'B', 0, 'L');
  ...
  public function getHeaderCuadroFinal($array = array(), $institucion = array())
  {
    ...
    $this->Cell(0, $height, 
      utf8_decode("REGISTRO DE EVALUACIÓN DEL RENDIMIENTO ESCOLAR DE ° GRADO DE EDUCACIÓN BÁSICA"), 0, 1
    );
  ...
  private function CuadroDeDatos(
    $array = array(), $limit = array()
  ) {
    $i = $limit['init'];
    $resultados = array();
    $this->SetFillColor(220);
    $this->SetFont('Arial','', 10);
    // $TercerNivel = intval($seccion['NIVEL']) > 6 ? true:false;

    for ($count = count($array); $i <= $limit['max'] && $i < ($count + 1); $i++) {
      self::row($array[$i-1], $i);
    }
  ...
  public function CuadroEstadistica($seccion = array())
  {
    $this->SetFillColor(210);
    $this->SetXY(26.0, 4.8);
